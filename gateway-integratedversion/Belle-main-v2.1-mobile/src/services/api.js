...
export const queueAPI = {
  ...
  async skipMatch(sessionId) {
    const query = `
      mutation SkipMatch($sessionId: ID!) {
...
export const chatAPI = {
  async getConversations(limit = 20, offset = 0) {
    ...
    const response = await fetch(
      `${config.COMMUNICATION_SERVICE_URL}/api/chat/conversations?limit=${limit}&offset=${offset}`,
      {
        headers: {
  ...
  async createOrGetConversation(participant2Id) {
    const token = await tokenStorage.getToken();
    const response = await fetch(
      `${config.COMMUNICATION_SERVICE_URL}/api/chat/conversations`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
...
export const notificationAPI = {
  ...
  async deleteAllNotifications(userId) {
    ...
    // userId not required by GraphQL mutation, but kept for compatibility
    const data = await graphqlClient.request(query, {}, true);
