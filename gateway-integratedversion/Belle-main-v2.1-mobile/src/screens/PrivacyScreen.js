...
export default function PrivacySafetyScreen({ navigation }) {
  const { sendPrivacySettingsUpdate } = useSocket();
  // --- State for Toggles ---
    ...
    userAPI
      ...
      .then((settings) => {
        if (isMounted && settings) {
          ...
          setShowOnlineStatus(settings.showOnlineStatus !== undefined ? settings.showOnlineStatus : true);
          setSendReadReceipts(settings.sendReadReceipts !== undefined ? settings.sendReadReceipts : true);
        }
      })
      .catch((err) => {
  ...
  const toggleOnlineStatus = async (value) => {
    ...
    try {
      ...
      sendPrivacySettingsUpdate({
        showOnlineStatus: value,
      });
    } catch (error) {
      console.warn("Failed to update online status setting:", error);
    }
  };
  ...
  const toggleReadReceipts = async (value) => {
    ...
    try {
      ...
      console.warn("Failed to update read receipts setting:", error);
    }
  };
