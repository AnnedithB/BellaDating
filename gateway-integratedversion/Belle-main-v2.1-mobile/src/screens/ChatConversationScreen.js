...
const ChatConversationScreen = ({ route }) => {
  ...
  // Mark session as read (if read receipts enabled)
  useEffect(() => {
    if (chatId && privacySettings.sendReadReceipts) {
      chatAPI.markSessionAsRead(chatId).catch(console.error);
    }
  }, [chatId, privacySettings.sendReadReceipts]);
  ...
  const handleUnmatch = () => {
    setShowMenu(false);
    if (otherUserId) {
      sendHeartUnmatch({ toUserId: otherUserId, roomId: chatId, sessionId: chatId });
    }
  };
