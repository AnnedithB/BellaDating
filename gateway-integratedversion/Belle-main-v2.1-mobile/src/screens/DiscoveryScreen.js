...
import { queueAPI, sessionAPI, chatAPI, userAPI } from '../services/api';
...
export default function DiscoveryScreen({ navigation, route }) {
  const { user } = useAuth();
  const { sendSkipMatch, onMatchSkipped } = useSocket();
  ...
  const handleMatchFound = async () => {
    ...
    try {
      ...
      if (sessions && sessions.length > 0) {
        ...
        const otherUser = latestSession.user1Id === user?.id ? latestSession.user2 : latestSession.user1;
        const otherUserId = otherUser?.id;

        // Fetch partner profile for UI details
        if (otherUserId) {
          userAPI
            .getUser(otherUserId)
            .then((u) => setPartnerProfile(u))
            .catch(() => setPartnerProfile(otherUser));
        } else {
          setPartnerProfile(otherUser);
        }

        // Ensure chat conversation exists and capture roomId
        if (otherUserId) {
          try {
            const conversation = await chatAPI.createOrGetConversation(otherUserId);
            const roomId = conversation?.roomId || conversation?.id || null;
            setConversationId(roomId);
            
            // Store the conversation in local state to ensure it persists
            // The conversation will now be visible in the Chats tab
            console.log('Chat conversation created/retrieved:', roomId);
          } catch (err) {
            console.warn('Failed to create conversation for match:', err);
  ...
  const handleNext = async () => {
    // Get the partner ID before ending the session
    const partnerId = currentSession ? (currentSession.user1Id === user?.id ? currentSession.user2Id : currentSession.user1Id) : null;
    
    try {
      if (currentSession?.id) {
        // Notify the other user that we're skipping
        if (partnerId) {
          sendSkipMatch({ toUserId: partnerId, sessionId: currentSession.id });
        }
      ...
      // Only start matching again if we weren't the one who was skipped
  ...
  const handleSkippedByPartner = useCallback(async () => {
    wasSkippedRef.current = true;
    Alert.alert('Match Ended', 'Your match has moved to the next person.');
    await endCall();
    setConversationId(null);
    setPartnerProfile(null);
    // Auto-start matching for the skipped user as well
    startMatching();
  }, []);
  ...
  // Listen for skip events from the other user
  useEffect(() => {
    if (!onMatchSkipped) return;
    
    const unsubscribe = onMatchSkipped((data) => {
      // Check if this skip event is for our current session
      if (currentSession?.id && data?.sessionId === currentSession.id) {
        handleSkippedByPartner();
      }
    });
  ...
  }, [currentSession?.id, onMatchSkipped, handleSkippedByPartner]);
  ...
  if (isInCall) {
    return (
      <CallView
        ...
        partnerId={currentSession ? (currentSession.user1Id === user?.id ? currentSession.user2Id : currentSession.user1Id) : null}
