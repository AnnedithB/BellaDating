...
const THEME_STORAGE_KEY = 'belle-selected-theme';

const ThemeToggler = () => {
  const [open, setOpen] = useState(false);
  const [anchorEl, setAnchorEl] = useState<null | HTMLElement>(null);
  const [selectedTheme, setSelectedTheme] = useState(
    getItemFromStore(THEME_STORAGE_KEY, presetOptions[0]?.value || 'default-light'),
  );
  ...
  const handleThemeSelect = (themeValue: string) => {
    ...
    setItemToStore(THEME_STORAGE_KEY, themeValue);
    handleOpen();
    handleMenuClose();
  };
  return (
