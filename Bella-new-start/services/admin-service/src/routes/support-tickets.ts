...
router.post('/:id/assign',
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const { id } = req.params;
      const { assignedTo, assignToSelf } = req.body;
      const adminId = (req as any).admin.id;
      
      // If assignToSelf is true, assign to current admin
      const targetAdminId = assignToSelf ? adminId : assignedTo;
      
      if (!targetAdminId) {
        return res.status(400).json({
          status: 'error',
          message: 'Either assignedTo or assignToSelf must be provided',
        });
      }
      ...
      // Log the assignment
      await logTicketActivity(id, adminId, 'ASSIGNED', { assignedTo: targetAdminId });
...
router.put('/:id/resolve',
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const resolutionTimeHours = (Date.now() - ticket.createdAt.getTime()) / (1000 * 60 * 60);
      ...
      // Log the resolution
      await logTicketActivity(id, adminId, 'RESOLVED', { resolution });
