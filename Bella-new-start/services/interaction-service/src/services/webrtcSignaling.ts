...
export class WebRTCSignalingService {
  ...
  private async handleVideoRequest(socket: Socket, data: VideoControlEvent): Promise<void> {
    try {
      ...
      // Validate that both users are in an active audio call
      // Check that the interaction exists and is in CONNECTED status
      const interaction = await this.prisma.interaction.findUnique({
        where: { roomId: data.roomId }
      });

      if (!interaction) {
        socket.emit('error', { message: 'No active call found. Video requests can only be sent during an active audio call.' });
        return;
      }

      if (interaction.status !== 'CONNECTED') {
        socket.emit('error', { message: 'Call is not connected. Video requests can only be sent during an active audio call.' });
        return;
      }

      // Ensure both users are in the room
      if (room.participants.size < 2) {
        socket.emit('error', { message: 'Both participants must be in the call to request video.' });
        return;
      }
      ...
      // Only emit to the call room, not user room, so only users in active call receive it
      this.io.to(data.roomId).emit('video-requested', {
        roomId: data.roomId,
  ...
  private async handleVideoAcceptance(socket: Socket, data: VideoControlEvent): Promise<void> {
    try {
      ...
      if (!interaction || interaction.status !== 'CONNECTED') {
        socket.emit('error', { message: 'Call is not connected. Video can only be enabled during an active audio call.' });
        logger.warn(`Video acceptance attempted for non-connected call: roomId=${data.roomId}, status=${interaction?.status}`);
        return;
      }
  ...
  private async handleVideoRejection(socket: Socket, data: VideoControlEvent): Promise<void> {
    try {
      ...
      if (!interaction || interaction.status !== 'CONNECTED') {
        socket.emit('error', { message: 'Call is not connected. Video requests can only be rejected during an active audio call.' });
        logger.warn(`Video rejection attempted for non-connected call: roomId=${data.roomId}, status=${interaction?.status}`);
        return;
      }
