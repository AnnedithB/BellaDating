...
router.get('/system', requireAdmin, asyncHandler(async (req: AuthenticatedRequest, res: express.Response) => {
  ...
  // Get user activity (handle missing table gracefully)
  let userActivity: any[] = [];
  try {
    userActivity = await (prisma.userAction.groupBy as any)({
      by: ['actionType'],
      where: {
        timestamp: {
          gte: start,
    ...
    if (dbError.message && dbError.message.includes('does not exist')) {
      logger.warn('User actions table does not exist. User activity data will be empty.');
    } else {
      logger.error('Error fetching user activity:', dbError);
    }
  }
