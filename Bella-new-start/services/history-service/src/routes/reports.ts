...
router.post('/', asyncHandler(async (req: AuthenticatedRequest, res: express.Response) => {
  ...
  // Log user action (non-critical, handle gracefully if table doesn't exist)
  try {
    ...
    // Log warning but don't fail the report creation
    if (actionError.message && actionError.message.includes('does not exist')) {
      logger.warn('User actions table does not exist. Action logging skipped.', {
        reportId: report.id,
        userId: req.user!.userId
      });
    } else {
      logger.error('Failed to log user action for report:', actionError);
    }
  }
