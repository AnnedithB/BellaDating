...
router.post('/actions', asyncHandler(async (req: AuthenticatedRequest, res: express.Response) => {
  ...
  try {
    ...
    // Handle missing table gracefully - action logging is non-critical
    if (dbError.message && dbError.message.includes('does not exist')) {
      logger.warn('User actions table does not exist. Please run migrations.', {
        error: dbError.message,
        userId: value.userId,
        actionType: value.actionType
      });
      
      // Return success anyway - action logging shouldn't break the app
      res.status(201).json({
        success: true,
        data: {
          id: 'skipped',
          message: 'Action logging skipped - table not found. Please run migrations.'
        }
      });
    } else {
