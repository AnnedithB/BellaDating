...
export class SocketService {
  ...
  private setupSocketEvents(socket: AuthenticatedSocket): void {
    ...
    socket.on('call:request', (data: { toUserId: string; callerName?: string; callerProfilePicture?: string }) => {
  ...
  private async handleCallRequest(
    socket: AuthenticatedSocket,
    data: { toUserId: string; callerName?: string; callerProfilePicture?: string; sessionId?: string }
  ): Promise<void> {
    try {
      if (!socket.userId) return;
      const toUserId = data?.toUserId;
