...
export class DatingMatchingAlgorithm {
   ...
   * Normalized to ensure perfect matches approach 100%
   */
  private static calculateWeightedDatingScore(
    ...
    // Fixed weights for additional factors
    const fixedEthnicityWeight = 0.02;
    const fixedFamilyPlansWeight = 0.08;
    const fixedReligionWeight = 0.05;
    const fixedEducationWeight = 0.03;
    const fixedPoliticalWeight = 0.03;
    ...
    const totalWeight = (
      ...
      avgLanguageWeight +
      avgGenderWeight +
      avgRelationshipWeight +
      avgLifestyleWeight +
      fixedEthnicityWeight +
      fixedFamilyPlansWeight +
      fixedReligionWeight +
      fixedEducationWeight +
      fixedPoliticalWeight
    );

    // Normalize weights to sum to 1.0 to ensure perfect matches approach 100%
    const normalizedAgeWeight = avgAgeWeight / totalWeight;
    const normalizedLocationWeight = avgLocationWeight / totalWeight;
    const normalizedInterestWeight = avgInterestWeight / totalWeight;
    const normalizedLanguageWeight = avgLanguageWeight / totalWeight;
    const normalizedGenderWeight = avgGenderWeight / totalWeight;
    const normalizedRelationshipWeight = avgRelationshipWeight / totalWeight;
    const normalizedLifestyleWeight = avgLifestyleWeight / totalWeight;
    const normalizedEthnicityWeight = fixedEthnicityWeight / totalWeight;
    const normalizedFamilyPlansWeight = fixedFamilyPlansWeight / totalWeight;
    const normalizedReligionWeight = fixedReligionWeight / totalWeight;
    const normalizedEducationWeight = fixedEducationWeight / totalWeight;
    const normalizedPoliticalWeight = fixedPoliticalWeight / totalWeight;

    // Calculate normalized weighted score (now sums to 1.0 when all scores are 1.0)
    const baseScore = (
      scores.ageScore * normalizedAgeWeight +
      scores.locationScore * normalizedLocationWeight +
      scores.interestScore * normalizedInterestWeight +
      scores.languageScore * normalizedLanguageWeight +
      scores.genderCompatScore * normalizedGenderWeight +
      scores.relationshipIntentScore * normalizedRelationshipWeight +
      scores.lifestyleScore * normalizedLifestyleWeight +
      scores.ethnicityScore * normalizedEthnicityWeight +
      scores.familyPlansScore * normalizedFamilyPlansWeight +
      scores.religionScore * normalizedReligionWeight +
      scores.educationScore * normalizedEducationWeight +
      scores.politicalScore * normalizedPoliticalWeight
    );
    ...
    const averageScore = (
      ...
      scores.genderCompatScore +
      scores.relationshipIntentScore +
      scores.lifestyleScore +
      scores.ethnicityScore +
      scores.familyPlansScore +
      scores.religionScore +
      scores.educationScore +
      scores.politicalScore
    ) / 13; // Average of all 13 scores

    // If average compatibility is high (>= 0.7), apply a boost
    // This helps ensure that when most things align, the score reflects that
    let compatibilityBoost = 0;
    if (averageScore >= 0.8) {
      compatibilityBoost = 0.15; // Strong boost for very compatible matches
    } else if (averageScore >= 0.7) {
      compatibilityBoost = 0.10; // Moderate boost for good matches
    } else if (averageScore >= 0.6) {
      compatibilityBoost = 0.05; // Small boost for decent matches
    }

    // Add premium bonus (capped at 0.1 to not overwhelm the base score)
    const premiumBonus = Math.min(0.1, scores.premiumBonus);
    
    // Final score: base score + compatibility boost + premium bonus
    // This ensures that good matches (where most factors align) get a higher score
    return Math.min(1.0, baseScore + compatibilityBoost + premiumBonus);
  }
  ...
  static async findBestDatingMatches(
    ...
    try {
      ...
      const matches = candidatesData
        ...
        .filter(match => {
          // Lower threshold - gender compatibility is not required
          // Users choose who they want to date, so we don't enforce gender compatibility
          return match.score.totalScore > 0.1; // Very low threshold to allow most matches
        })
        .sort((a, b) => {
