...
export class AdvancedMatchingAlgorithm {
  ...
  private static calculateInterestScore(user1: UserMatchData, user2: UserMatchData): number {
    ...
    // If no interests specified, return neutral score (don't penalize)
    if (user1Interests.size === 0 && user2Interests.size === 0 && user1Preferred.size === 0 && user2Preferred.size === 0) {
      return 0.5; // Neutral score if no interest data
    }
  ...
  private static calculateLanguageScore(user1: UserMatchData, user2: UserMatchData): number {
    ...
    // If no languages specified, return neutral score (don't penalize)
    if (user1Languages.size === 0 && user2Languages.size === 0) {
    ...
    // If no common language but languages are specified, return low score
    if (commonLanguages.size === 0 && (user1Languages.size > 0 || user2Languages.size > 0)) {
      return 0.2; // Low score but not zero to allow matching
    }
  ...
  static async findBestMatches(
    ...
    try {
      ...
      const matches = candidatesData
        ...
        .filter(match => match.score.totalScore > 0.1) // Lower threshold - gender compatibility not required
