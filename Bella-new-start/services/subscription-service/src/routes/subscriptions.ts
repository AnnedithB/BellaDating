...
router.post('/create-checkout-session',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const webYearlyPrice = 99.99;
    
    // Use web pricing for Stripe checkout
    const webPrice = billingCycle === 'YEARLY' ? webYearlyPrice : webMonthlyPrice;
    const priceInterval = billingCycle === 'YEARLY' ? 'year' : 'month';
    ...
    try {
      const stripePrice = await stripe.prices.create({
        ...
        product_data: {
          name: `${plan.displayName} - ${billingCycle === 'YEARLY' ? 'Yearly' : 'Monthly'} (Web)`,
        },
      });
    ...
    try {
      ...
      if (session.payment_status === 'paid' && session.subscription) {
        ...
        if (subscription) {
          ...
          // Create subscription
          const price = billingCycle === 'YEARLY' ? 99.99 : 19.99;
