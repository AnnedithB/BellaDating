...
export async function getSubscriptionStatus() {
  try {
    ...
    const url = `${config.SUBSCRIPTION_SERVICE_URL}/api/apple-iap/subscription-status`;
    ...
    if (response.status === 429) {
      console.warn('[IAP] ⚠️ Rate limited (429). Too many requests.');
      // Return cached result if available, or throw a rate limit error
    ...
    try {
      ...
      // If response is not JSON (e.g., plain text error), handle it
      if (!response.ok) {
    ...
    console.error('[IAP] ❌❌❌ BACKEND STATUS ERROR! ❌❌❌');
