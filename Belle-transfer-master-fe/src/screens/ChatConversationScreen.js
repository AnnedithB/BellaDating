...
const ChatConversationScreen = ({ route }) => {
  ...
  const handleStartVoiceCall = async (isIncoming = false) => {
    ...
    try {
      ...
      // Initialize WebRTC for voice (false for isVideo)
  ...
  useEffect(() => {
    const unsubscribe = subscribeToCallResponses(async (data) => {
      ...
      const isActiveSession = callSession && (
        data.callId === callSession.id ||
        data.sessionId === callSession.id ||
      ...
      if (isActiveSession) {
        if (data.response === 'accept') {
          ...
          const isCaller = data.callerId === user?.id;
          const targetId = callSession?.partnerId || actualOtherUserId;

          if (isCaller && targetId) {
            console.log('[ChatConversationScreen] We are the caller, creating offer for:', targetId);
            try {
              // Peer connection should have been created in initializeVideoCall
          ...
          // Both decline and ignore should navigate back and restart matching
          console.log('[ChatConversationScreen] Call was', data.response === 'decline' ? 'declined' : 'ignored');
          cleanupVideoCall(true); // Navigate back to Discovery and restart matching
        }
      }
    });
  ...
  const handleEndCall = async () => {
    try {
      ...
      // cleanupVideoCall will notify the other party via callService.endCall()
      // Pass true to navigate back to Discovery and restart matching
  ...
  // Show Video Call Interface when in call (Priority over loading)
  if (isInCall && callSession) {
    console.log('[ChatConversationScreen] Rendering video call interface, status:', callStatus, 'callSession:', {
    ...
    // Determine if we should show video streams (only when connected and remote stream is available)
    ...
    return (
      <View style={{ flex: 1, backgroundColor: '#000' }}>
        {isConnected ? (
          // Show video streams when connected
          <>
