...
export default function SubscriptionScreen({ navigation }) {
  ...
  const [billingCycle, setBillingCycle] = useState('MONTHLY'); // 'MONTHLY' or 'SIXMONTH'
  ...
  const handleSubscribe = async () => {
    ...
    const productId = billingCycle === 'SIXMONTH'
      ? plan.appleProductIdYearly // Use yearly product ID for 6 months
    ...
    if (iapAvailable && productId && Platform.OS === 'ios') {
      ...
      const priceDisplay = billingCycle === 'SIXMONTH'
        ? (plan.yearlyPriceDisplay || `$${plan.yearlyPrice || 99.99}`)
        : (plan.monthlyPriceDisplay || `$${plan.monthlyPrice}`);

      const billingPeriod = billingCycle === 'SIXMONTH' ? '6 months' : billingCycle === 'YEARLY' ? 'year' : 'month';
      
      Alert.alert(
        'Subscribe to ' + plan.displayName,
        `You will be charged ${priceDisplay}/${billingPeriod}.\n\n${
          iapAvailable
  ...
  const getPrice = (plan) => {
    if (billingCycle === 'SIXMONTH') {
      return plan.yearlyPriceDisplay || `$${plan.yearlyPrice || 99.99}`;
    }
  ...
  const getPriceAmount = (plan) => {
    if (billingCycle === 'SIXMONTH') {
      return plan.yearlyPriceAmount || plan.yearlyPrice || 99.99;
    }
          ...
          {isFree ? (
            ...
            <>
              ...
              <Text style={styles.pricePeriod}>
                /{billingCycle === 'SIXMONTH' ? '6 months' : billingCycle === 'YEARLY' ? 'year' : 'month'}
              </Text>
            </>
          )}
        </View>

        {!isFree && billingCycle === 'SIXMONTH' && plan.yearlySavings > 0 && (
          <View style={styles.savingsBadge}>
            <Text style={styles.savingsText}>
              Save ${plan.yearlySavings.toFixed(2)}/6 months
            </Text>
          </View>
        )}
      ...
      >
        ...
        <View style={styles.billingToggleContainer}>
          ...
          <TouchableOpacity
            style={[
              styles.billingOption,
              billingCycle === 'SIXMONTH' && styles.billingOptionSelected,
            ]}
            onPress={() => setBillingCycle('SIXMONTH')}
          >
            <Text
              style={[
                styles.billingOptionText,
                billingCycle === 'SIXMONTH' && styles.billingOptionTextSelected,
              ]}
            >
              6 Months
            </Text>
