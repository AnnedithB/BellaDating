...
export default function SubscriptionSuccessScreen({ route }) {
  ...
  useEffect(() => {
    ...
    if (sessionId) {
      ...
      console.log('[SubscriptionSuccess] ⚠️ No session ID found, but will poll anyway in case payment completed via webhook...');
      // Even without session_id, poll for subscription (webhook might have already created it)
  ...
  const verifySessionAndPoll = async (sessionId) => {
    ...
    // If verification didn't find it, start polling
    console.log('[SubscriptionSuccess] ========== STARTING POLLING ==========');
    pollSubscriptionStatus();
  };
  ...
  const pollSubscriptionStatus = async (maxAttempts = 20, delay = 1500) => {
    // Prevent re-running if already successful
    if (hasSucceeded.current) {
      console.log('[SubscriptionSuccess] ⚠️ Already successful, skipping polling...');
      return;
    }
    
    console.log('[SubscriptionSuccess] ========== POLL SUBSCRIPTION STATUS START ==========');
    console.log('[SubscriptionSuccess] Max attempts:', maxAttempts);
    console.log('[SubscriptionSuccess] Delay between attempts:', delay);
    
    for (let i = 0; i < maxAttempts; i++) {
      // Check if we've already succeeded during polling
      if (hasSucceeded.current) {
        console.log('[SubscriptionSuccess] ⚠️ Success detected during polling, stopping...');
        return;
      }
      try {
        console.log(`[SubscriptionSuccess] ========== POLLING ATTEMPT ${i + 1}/${maxAttempts} ==========`);
        ...
        console.log('[SubscriptionSuccess] subscriptionStatus:', subscriptionStatus);
        console.log('[SubscriptionSuccess] subscriptionStatus?.subscription:', subscriptionStatus?.subscription);
        ...
        if (subscriptionStatus?.subscription?.status === 'ACTIVE') {
          ...
          console.log('[SubscriptionSuccess] ✅✅✅ ACTIVE SUBSCRIPTION FOUND IN POLLING! ✅✅✅');
          ...
          console.log('[SubscriptionSuccess] Full subscription object:', subscriptionStatus?.subscription);
        }
      } catch (e) {
        console.log(`[SubscriptionSuccess] ❌ Polling attempt ${i + 1} ERROR!`);
        console.log('[SubscriptionSuccess] Error message:', e.message);
        console.log('[SubscriptionSuccess] Error stack:', e.stack);
        console.log('[SubscriptionSuccess] Full error:', e);
      }
    ...
    console.log('[SubscriptionSuccess] ========== POLLING COMPLETED - NO ACTIVE SUBSCRIPTION FOUND ==========');
    console.log('[SubscriptionSuccess] ❌ Polling completed but no active subscription found after', maxAttempts, 'attempts');
