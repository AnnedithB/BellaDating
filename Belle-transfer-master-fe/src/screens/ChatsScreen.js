...
export default function ChatsScreen({ navigation }) {
  const { user } = useAuth();
  const { subscribeToMatches, getTypingUsersInConversation, typingUsers } = useSocket();
  ...
  const loadSessions = async (showLoading = true) => {
    try {
      ...
      // Add typing status to each chat
  ...
  // Update typing status when typingUsers changes
  useEffect(() => {
    if (sessions.length > 0 && typingUsers) {
