...
export default function LocationModal({ visible, onClose, onLocationSelect }) {
  ...
  const searchLocations = async () => {
    ...
    try {
      // Use backend endpoint to proxy Nominatim API (avoids CORS issues)
      const response = await fetch(
        `${config.API_URL}/profile/search-location?q=${encodeURIComponent(searchQuery)}`,
        {
          method: 'GET',
          headers: {
      ...
      // Extract locations from backend response
      const formattedSuggestions = result.data?.locations || [];
